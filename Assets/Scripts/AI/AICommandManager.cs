...
namespace SandwichGame.AI
{
    public class AICommandManager:MonoBehaviour
    {
        ...
        public void Interpret(string command)
        {
            ...
            if (string.IsNullOrEmpty(command) || command.Length > 200)
            {
                RequestStateChanged?.Invoke(false, "명령은 1~200자로 입력하세요.");
                return;
            }

            if (!ValidateMayonnaiseCommand(command, out string hint))
            {
                executor?.RejectCommand(hint);
                RequestStateChanged?.Invoke(false, "COMMAND FAILED");
                return;
            }

            StartCoroutine(Run(command));
        }

        private static bool ValidateMayonnaiseCommand(
            string command,
            out string hint)
        {
            hint = null;

            bool targetsMayonnaise =
                command.Contains("마요네즈") ||
                command.IndexOf(
                    "mayonnaise",
                    StringComparison.OrdinalIgnoreCase) >= 0;

            if (!targetsMayonnaise) return true;

            bool explicitlyOpensLid =
                command.Contains("뚜껑") && command.Contains("열");

            if (!explicitlyOpensLid)
            {
                hint = "마요네즈 뚜껑을 열고 짜주세요.";
                return false;
            }

            bool specifiesAmount =
                command.Contains("많이") ||
                command.Contains("적당히") ||
                command.Contains("조금");

            if (!specifiesAmount)
            {
                hint = "마요네즈 양을 많이, 적당히 또는 조금으로 말해주세요.";
                return false;
            }
        ...
        private static string BuildMockResponse(string command)
        {
            if((command.Contains("양배추")||command.ToLowerInvariant().Contains("cabbage"))&&(command.Contains("자르")||command.Contains("잘라")||command.Contains("썰")))
            {
                ...
                string put=(command.Contains("올려")||command.Contains("놓아")||command.Contains("넣어"))
                    ? ",{\"action\":\"Put\",\"targetIngredient\":\"cabbage\",\"amount\":null,\"rawCommandPart\":\""+EscapeJson(command)+"\"}"
                    : string.Empty;
                return "{\"actions\":[{\"action\":\"Cut\",\"targetIngredient\":\"cabbage\",\"amount\":\""+amount+"\",\"rawCommandPart\":\""+EscapeJson(command)+"\"}"+put+"]}";
            }
